...
init_project() {
    ...
    # Copy universal agents (always included)
    local agents_source="$SYNAPSE_HOME/.synapse/agents"
    local universal_agents=(
    ...
    for agent in "${universal_agents[@]}"; do
        if [[ -f "$agents_source/${agent}.md" ]]; then
            cp "$agents_source/${agent}.md" "$project_dir/.claude/agents/"
            success "Added $agent agent"
        else
            warning "${agent}.md not found in $agents_source"
        fi
    done
...
EOF
    ...
    info "  @code-hound review my code for quality issues"
    info "  @git-workflow create feature branch and PR"
    info "  @test-runner execute tests and analyze failures"
    info "  @file-creator setup project templates"
    echo
